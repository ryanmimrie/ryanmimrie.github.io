...
<ol>
  <li><b>Designer:</b> In this stage, the user provides information on the behaviours and parameter values to be used in the model run. The user may also provide experimental data for the model to be fit to, and specify which parameters they would like the model to estimate during data fitting. All of this information is stored in a .mavi text file, which can be saved for running later, shared with other researchers, or published alongside findings for reproducibility. </li>
  <br>
  <li><b>Solver:</b> In this stage, the model(s) specified in the .mavi text file are solved by the WH-MAVI engine. When there is not any data for fitting, the engine will simply run every theoretical scenario specified in the file. If the .mavi file includes data for fitting, the engine will run many iterations of the model with different values for the parameters the user has asked to be estimated. In this way, the engine will attempt to provide the most likely values of these parameters given the experimental data provided, as well as the level of uncertainty in these values.</li>
</ol>
...
parameterE: (value_for_model_1, value_for_model_2)
{% endhighlight %}

Some behaviours to be aware of:
<ul>
  <li>Each parameter should be specified on a separate line.</li>
  <li>Whitespace, with the exception of newlines, will be ignored.</li>
...
{% highlight python %}
# A parameter with different values two model iterations.
